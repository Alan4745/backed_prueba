...
const createNewNote = async (req, res) => {
    try {
        ...
        if (!userSender) {
            return res.status(404).json({ message: 'No se encontró al usuario remitente' });
        }
        if (!userReciver) {
...
const getSentNotesByUser = async (req, res) => {
    try {
        ...
        if (!notes.length) {
            return res.status(404).json({ message: 'No se encontraron notas enviadas por este usuario' });
        }
...
const getReceivedNotesByUser = async (req, res) => {
    try {
        ...
        if (!notes.length) {
            return res.status(404).json({ message: 'No se encontraron notas recibidas por este usuario' });
        }

        res.status(200).json({ notes: applyAnonymity(notes) });
    } catch (error) {
        console.error('Error al obtener notas recibidas por el usuario:', error);
        res.status(500).json({ message: 'Error al obtener notas recibidas', error });
    }
};
