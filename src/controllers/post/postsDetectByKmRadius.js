...
const postsDetectByKmRadius = async (req, res) => {
  try {
    ...
    const filteredPosts = await Promise.all(
      allPosts.map(async (post) => {
        ...
        if (
          ...
          const verificationResult = await verifyRedemptionPost(
            post,
            coordinates,
            radiusInMeters
          );
          if (verificationResult.success) {
            ...
            let collectionFound = await Collections.findOne({
              idPost: post._id,
            });
            if (!collectionFound) {
            ...
            const ticketsFounds = await TokenCollection.find({
              idCollection: collectionFound._id,
            });
            const numberOfTickets =
              ticketsFounds.length > 0 ? ticketsFounds.length : 0;
    ...
    res.status(500).json({
      success: false,
      message: "Error al buscar posts en el radio especificado",
    });
  }
};
